...
use pocketmine\utils\ServerOptimizerSettings;
...
class World implements ChunkManager{
	...
	public function __construct(
		...
		if($cfg->getPropertyInt("chunk-ticking.per-tick", 40) <= 0){
			...
			$this->logger->warning("\"chunk-ticking.per-tick\" setting is deprecated, but you've used it to disable chunk ticking. Set \"chunk-ticking.tick-radius\" to 0 in \"cloudpm.yml\" instead.");
	...
	protected function actuallyDoTick(int $currentTick) : void{
		...
		foreach($this->updateEntities as $id => $entity){
			...
			/* Coming Soon
			if(!ServerOptimizerSettings::$tickEntities && !$entity instanceof Player){
				continue;
			}

			if($entity instanceof Animal && !ServerOptimizerSettings::$tickAnimals){
				continue;
			}

			if($entity instanceof Zombie && !ServerOptimizerSettings::$tickMonsters){
				continue;
			}

			if($entity instanceof ItemEntity && !ServerOptimizerSettings::$tickDrops){
				continue;
			}
			*/
