...
class Server{
	...
	public function getCloudPMVersion() : string{
	...
	public function __construct(
		...
		try{
			...
			$pocketmineYmlPath = Path::join($this->dataPath, "cloudpm.yml");
			if(!file_exists($pocketmineYmlPath)){
				$content = Filesystem::fileGetContents(Path::join(\pocketmine\RESOURCE_PATH, "cloudpm.yml"));
			...
			$this->logger->info($this->language->translate(KnownTranslationFactory::pocketmine_cloud_info(
				$this->getName(),
				(VersionInfo::IS_DEVELOPMENT_BUILD ? TextFormat::YELLOW : "") . $this->getCloudPMVersion() . TextFormat::RESET
			)));
