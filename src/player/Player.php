...
class Player extends Human implements CommandSender, ChunkListener, IPlayer, NeverSavedWithChunkEntity{
	...
	private ChunkSendQueue $chunkSendQueue;
	...
	public function __construct(Server $server, NetworkSession $session, PlayerInfo $playerInfo, bool $authenticated, Location $spawnLocation, ?CompoundTag $namedtag){
		...
		$this->chunkSendQueue = new ChunkSendQueue($this);
		...
		foreach($this->loadQueue as $index => $distance){
			...
			$world->requestChunkPopulation($X, $Z, $this->chunkLoader)->onCompletion(
				function() use ($X, $Z, $index, $world) : void{
					...
					$priority = $this->calculateChunkPriority($X, $Z);
					$this->chunkSendQueue->queueChunk($X, $Z, $priority);
				},
	...
	private function calculateChunkPriority(int $chunkX, int $chunkZ) : int{
	...
	/**
	 * Immediately send the chunk (chunkX, chunkZ) to the client.
	 */
	public function sendChunkNow(int $chunkX, int $chunkZ) : void{
		...
		$this->getNetworkSession()->startUsingChunk($chunkX, $chunkZ, function() use ($chunkX, $chunkZ, $index) : void{
			if(!$this->isConnected()){
				return;
			}
			if(!isset($this->usedChunks[$index])){
			...
			(new PlayerPostChunkSendEvent($this, $chunkX, $chunkZ))->call();
		});
	}
	...
	public function doChunkRequests() : void{
		...
		$this->chunkSendQueue->process($this->chunksPerTick);
	}
